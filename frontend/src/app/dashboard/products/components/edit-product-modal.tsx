...
import { editProductSchema, type EditProductFormValues } from '../products.schema';
...
interface EditProductModalProps {
  ...
  onSubmit: (data: EditProductFormValues) => Promise<void>;
}
...
export function EditProductModal({ isOpen, onClose, product, onSubmit }: EditProductModalProps) {
  ...
  React.useEffect(() => {
    if (isOpen && product) {
      reset({
        ...
        gender: getGenderLabel(product.gender),
        detail_product_group: product.detail_product_group,
        size: Number(product.size),
        age_group: product.age_group,
        activity_group: product.activity_group,
        lifestyle_group: product.lifestyle_group,
      });
    }
  }, [isOpen, product, reset]);

  const handleFormSubmit = async (data: EditProductFormValues) => {
    try {
      await onSubmit(data);
      onClose();
    } catch (error) {
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={`Cập nhật sản phẩm: ${product?.product_id || ''}`}
    >
      <FormProvider {...form}>
        <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-4">
          <FieldGroup className="gap-3">
            <div className="grid grid-cols-2 gap-3">
              <Field>
                <FieldLabel htmlFor="edit-product-color">Màu sắc</FieldLabel>
                <Input
              ...
              <Field>
                <FieldLabel htmlFor="edit-product-size">Kích cỡ (Size)</FieldLabel>
                <Input
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field>
                <FieldLabel htmlFor="edit-product-price-cost">Giá vốn (VNĐ)</FieldLabel>
                <Input
                  id="edit-product-price-cost"
                  type="number"
              ...
              <Field>
                <FieldLabel htmlFor="edit-product-listing-price">Giá bán lẻ (VNĐ)</FieldLabel>
                <Input
                  id="edit-product-listing-price"
                  type="number"
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field>
                <FieldLabel htmlFor="edit-product-gender">Giới tính</FieldLabel>
                <Controller
                  name="gender"
                  control={control}
                  render={({ field }) => (
                    <Select onValueChange={field.onChange} value={field.value}>
                      <SelectTrigger id="edit-product-gender" className="w-full">
              ...
              <Field>
                <FieldLabel htmlFor="edit-product-detail-group">Nhóm chi tiết sản phẩm</FieldLabel>
                <Controller
                  name="detail_product_group"
                  control={control}
                  render={({ field }) => (
                    <Select onValueChange={field.onChange} value={field.value}>
                      <SelectTrigger id="edit-product-detail-group" className="w-full">
            ...
            <Field>
              <FieldLabel htmlFor="edit-product-age-group">Nhóm độ tuổi</FieldLabel>
              <Controller
                name="age_group"
                control={control}
                render={({ field }) => (
                  <Select onValueChange={field.onChange} value={field.value}>
                    <SelectTrigger id="edit-product-age-group" className="w-full">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field>
                <FieldLabel htmlFor="edit-product-activity-group">Nhóm hoạt động</FieldLabel>
                <Controller
                  name="activity_group"
                  control={control}
                  render={({ field }) => (
                    <Select onValueChange={field.onChange} value={field.value}>
                      <SelectTrigger id="edit-product-activity-group" className="w-full">
              ...
              <Field>
                <FieldLabel htmlFor="edit-product-lifestyle-group">Nhóm phong cách sống</FieldLabel>
                <Controller
                  name="lifestyle_group"
                  control={control}
                  render={({ field }) => (
                    <Select onValueChange={field.onChange} value={field.value}>
                      <SelectTrigger id="edit-product-lifestyle-group" className="w-full">
