...
interface EditInventoryReportModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: EditInventoryReportFormValues) => Promise<void>;
...
export function EditInventoryReportModal({ isOpen, onClose, onSubmit, report }: EditInventoryReportModalProps) {
  ...
  const form = useForm<EditInventoryReportFormValues>({
    resolver: zodResolver(editInventoryReportSchema),
  });
  ...
  React.useEffect(() => {
    if (isOpen && report) {
      ...
      reset({
        ...
        calendar_year_week: report.calendar_year_week ? report.calendar_year_week.slice(0, 10) : '',
        quantity: report.quantity,
      });
    }
  }, [isOpen, report, reset]);

  const handleFormSubmit = async (data: EditInventoryReportFormValues) => {
    try {
      await onSubmit(data);
      onClose();
    } catch (error) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Cập nhật báo cáo tồn kho">
      {isLoadingDropdowns ? (
        ...
        <FormProvider {...form}>
          <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-4">
            <FieldGroup className="gap-3">
              {/* Chọn sản phẩm */}
              <Field>
                <FieldLabel htmlFor="edit-inv-product">Sản phẩm</FieldLabel>
                <Controller
                  name="product_id"
                  control={control}
                  render={({ field }) => (
                    <Select onValueChange={field.onChange} value={field.value}>
                      <SelectTrigger id="edit-inv-product" className="w-full">
              ...
              {/* Chọn nhà máy */}
              <Field>
                <FieldLabel htmlFor="edit-inv-plant">Nhà máy sản xuất</FieldLabel>
                <Controller
                  name="plant_id"
                  control={control}
                  render={({ field }) => (
                    <Select onValueChange={field.onChange} value={field.value}>
                      <SelectTrigger id="edit-inv-plant" className="w-full">
              ...
              <div className="grid grid-cols-2 gap-3">
                {/* Số lượng */}
                <Field>
                  <FieldLabel htmlFor="edit-inv-qty">Số lượng tồn kho</FieldLabel>
                  <Input
                    id="edit-inv-qty"
                    type="number"
                ...
                {/* Thời gian */}
                <Field>
                  <FieldLabel htmlFor="edit-inv-time">Thời gian (Tuần)</FieldLabel>
                  <Input
                    id="edit-inv-time"
                    type="date"
