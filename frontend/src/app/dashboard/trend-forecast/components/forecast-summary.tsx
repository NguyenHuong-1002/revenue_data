...
export function ForecastSummary({ sales, inventory, scope }: ForecastSummaryProps) {
  ...
  const getTrendMetrics = (data: IForecastDatasetResult | null) => {
    ...
    const avgLinearForecast =
      linearForecasts.length > 0
        ? linearForecasts.reduce((sum, item) => sum + item.value, 0) / linearForecasts.length
        : lastActual;

    const finalLinearForecast =
      linearForecasts.length > 0 ? linearForecasts[linearForecasts.length - 1].value : lastActual;
  ...
  const renderSalesCard = () => {
    ...
    if (isUp) {
      recommendation =
        'Xu hướng doanh số đang tăng trưởng tốt. Cần lên kế hoạch nhập thêm nguyên vật liệu và tối ưu năng lực sản xuất để đáp ứng nhu cầu thị trường.';
    } else if (isDown) {
      recommendation =
        'Dự báo doanh số có xu hướng giảm nhẹ. Cân nhắc triển khai các chương trình khuyến mãi kích cầu hoặc giảm sản lượng sản xuất để tránh tồn kho thừa.';
    } else {
      recommendation =
        'Doanh số dự kiến sẽ giữ ở mức ổn định. Tiếp tục theo dõi biến động thị trường và duy trì kế hoạch vận hành hiện tại.';
    }
          ...
          <div className="grid grid-cols-3 gap-3 bg-muted/20 p-3 rounded-xl border border-border/40">
            <div>
              ...
              <p className="text-base font-bold text-foreground mt-0.5">
                {Math.round(lastActual).toLocaleString()}{' '}
                <span className="text-xs font-normal text-muted-foreground">sp</span>
              </p>
            </div>
            <div className="border-l border-border/50 pl-3">
              <p className="text-[10px] text-muted-foreground font-medium">Dự báo TB tương lai</p>
              <p className="text-base font-bold text-primary mt-0.5">
                {Math.round(avgLinearForecast).toLocaleString()}{' '}
                <span className="text-xs font-normal text-muted-foreground">sp</span>
              </p>
            </div>
            <div className="border-l border-border/50 pl-3">
              <p className="text-[10px] text-muted-foreground font-medium">
                Tốc độ thay đổi (Slope)
              </p>
              <p
                className={`text-base font-bold mt-0.5 ${slope > 0 ? 'text-emerald-600 dark:text-emerald-400' : slope < 0 ? 'text-red-500' : 'text-foreground'}`}
              >
                {slope > 0 ? '+' : ''}
                {slope.toFixed(2)}{' '}
                <span className="text-[10px] font-normal text-muted-foreground">sp/kỳ</span>
              </p>
            </div>
          </div>
  ...
  const renderInventoryCard = () => {
    ...
    if (isAccumulating) {
      recommendation =
        'Lượng tồn kho đang tích tụ lũy kế. Hãy rà soát lại tiến độ đặt hàng từ nhà máy hoặc đẩy mạnh kênh bán lẻ để tối ưu chi phí lưu kho, tránh ứ đọng vốn.';
    } else if (isDepleting) {
      recommendation =
        'Mức tồn kho đang suy giảm nhanh. Cần chuẩn bị kế hoạch bổ sung nguồn cung (nhập kho mới) sớm để tránh nguy cơ đứt gãy chuỗi cung ứng, cháy hàng.';
    } else {
      recommendation =
        'Tồn kho được dự báo duy trì cân bằng ổn định. Tiếp tục áp dụng mức tồn kho an toàn (safety stock) hiện tại.';
    }
        ...
        <CardContent className="pt-6 pb-5 px-5 flex flex-col gap-4">
          ...
          <div className="grid grid-cols-3 gap-3 bg-muted/20 p-3 rounded-xl border border-border/40">
            ...
            <div className="border-l border-border/50 pl-3">
              ...
              <p
                className={`text-base font-bold mt-0.5 ${slope > 0 ? 'text-amber-500' : slope < 0 ? 'text-blue-600 dark:text-blue-400' : 'text-foreground'}`}
              >
                {slope > 0 ? '+' : ''}
                {slope.toFixed(2)}{' '}
                <span className="text-[10px] font-normal text-muted-foreground">sp/ngày</span>
              </p>
            </div>
          </div>
  ...
  return (
    <div
      className={`grid grid-cols-1 ${showSales && showInventory ? 'md:grid-cols-2' : 'grid-cols-1'} gap-6`}
    >
