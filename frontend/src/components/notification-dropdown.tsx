...
const MOCK_NOTIFICATIONS: INotification[] = [
  ...
  },
  {
    ...
    account_id: null,
    title: "Doanh số mới",
    content: "Đơn hàng mới! Khách hàng 098****321 đã mua sản phẩm SP173918 tại chi nhánh Hồ Chí Minh.",
    type: "NEW_SALE",
    read_at: null,
  ...
  },
  {
    ...
    content: "Admin hoangle đã tạo mới tài khoản staff_lananh (Nguyễn Thị Lan Anh) với vai trò STAFF.",
    type: "CUSTOMER_NEW",
    read_at: null,
  ...
  return (
    <DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
      <DropdownMenuTrigger asChild>
        ...
        >
          <BellIcon className={`size-5 text-foreground/80 ${unreadCount > 0 ? "animate-pulse" : ""}`} />
          {unreadCount > 0 && (
            <Badge
          ...
          {notifications.length === 0 ? (
            ...
            notifications.map((noti) => {
              ...
              return (
                ...
                >
                  {/* Icon loại thông báo */}
                  <div className={`size-8 shrink-0 flex items-center justify-center rounded-lg border ${config.bgColor}`}>
                    {config.icon}
                  </div>
